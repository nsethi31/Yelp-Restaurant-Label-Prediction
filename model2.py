...
np.save('../data/train_test_kmn.npy',tr_ts)
...
for num_cluster in ([2,3,4,5]):
	...
	train_image_features = np.load('../data/train_test_kmn.npy',mmap_mode = 'r')
	...
	for nb,lb in enumerate(labels):
		...
		if(cv):
			for (training,testing) in skf:
				...
				bst = xgb.train(param, xg_train, num_round,watchlist,verbose_eval=1,early_stopping_rounds=20)
	...
	np.save('../data/Model2_Full_'+str(num_cluster)+'.npy',dataset_blend_train)
	...
	if(submit):
		...
		np.save('../data/Model2_Full_'+str(num_cluster)+'_result.npy',result)
		...
		x1.to_csv('../data/result2.csv',index=0)
		print ("results saved")
		'''
